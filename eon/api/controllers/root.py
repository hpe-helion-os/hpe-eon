...
from eon.api.controllers import versions
...
class Version(base.APIBase):
    ...
    * DEPRECATED - supported, but deprecated, version of API.
    ...
    def __init__(self, _id, min_version, version, status='CURRENT'):
        self.id = _id
        self.links = [link.Link.make_link('self', pecan.request.host_url,
...
class Root(base.APIBase):
    ...
    def convert():
        root = Root()
        root.name = "eon API"
        root.description = "The cloud on boarder"
        root.default_version = Version(ID_VERSION2,
                                       versions.MIN_VERSION_STRING,
                                       versions.MIN_VERSION_STRING,
                                       status="INITIAL")
        root.version2 = Version(ID_VERSION2,
                                versions.MIN_VERSION_STRING,
                                ID_VERSION2)
        root.versions = [root.default_version, root.version2]
        return root


class RootController(rest.RestController):

    _versions = [ID_VERSION2]
    """All supported API versions"""

    _default_version = ID_VERSION2
    ...
    def _route(self, args):
        ...
        It redirects the request to the default version of the eon API
