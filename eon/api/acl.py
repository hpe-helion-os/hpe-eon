...
from eon.api.middleware import audit
from eon.api.middleware import auth_token
...
def install_auth(app, conf, public_routes):
...
def install_audit(app, conf, public_routes):
    """Install Audit Middleware on application

    :param app: A WSGI application
    :param conf: Settings
    :param public_routes: The list of the routes which will be allowed to
                          access without authentication.
    :return: The same WSGI application with Audit Middleware installed
    """

    return audit.AuditMiddleware(app, public_routes,
                                 audit_map_file=conf.audit_map_file)
