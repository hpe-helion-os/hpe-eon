...
util_opts = [
    ...
    cfg.StrOpt('lock_path',
               default=os.environ.get("EON_LOCK_PATH"),
        ...
        def inner(*args, **kwargs):
            try:
                with lock(name, lock_file_prefix, external, lock_path):
                    LOG.info('Got semaphore / lock "%(function)s"',
                              {'function': f.__name__})
                    return f(*args, **kwargs)
            finally:
                LOG.info('Semaphore / lock released "%(function)s"',
                          {'function': f.__name__})
        return inner
    return wrap
...
def main(argv):
    ...
    os.environ["EON_LOCK_PATH"] = lock_dir
    try:
